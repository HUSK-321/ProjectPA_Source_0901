...
UPlayerCameraComponent* UCameraMode::GetCameraComponent() const
{
...
void UCameraMode::OnActivation()
{
	UPlayerCameraComponent* CameraComponent = GetCameraComponent();
	if (IsValid(CameraComponent))
	{
		CameraComponent->bUsePawnControlRotation = bUsePawnControlRotation;
	}
	...
	if (const ACharacter* TargetCharacter = Cast<ACharacter>(GetTargetActor()))
	{
		...
		{
			TargetCharacter->GetCharacterMovement()->bOrientRotationToMovement = bOrientRotationToMovement;
		}
	}
}

void UCameraMode::OnDeactivation()
{
}

AActor* UCameraMode::GetTargetActor() const
{
	const UPlayerCameraComponent* CameraComponent = GetCameraComponent();

	return IsValid(CameraComponent) ? CameraComponent->GetTargetActor() : nullptr;
}
//~End UCameraMode

//~Begin UCameraModeStack
...
void UCameraModeStack::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	...
	{
		UCameraMode* TopEntry = CameraModeStack.Last();
		check(TopEntry);
